...
try:
    ...
    print(
        "Error: Unable to import AI workflow modules. Make sure you're running this script from the repository root."
    )
    sys.exit(1)
...
def execute_chain(args):
    ...
    if args.context:
        try:
            ...
            print(
                "Error: Invalid JSON context. Please provide context as a valid JSON string."
            )
            sys.exit(1)
...
def analyze_diff(args):
    ...
    report = analyzer.generate_diff_report(
        [analysis], args.marker, verification, args.iteration
    )
    ...
    if args.update:
        ...
        if args.metrics:
            try:
                ...
                print(
                    "Error: Invalid JSON metrics. Please provide metrics as a valid JSON string."
                )
                sys.exit(1)
...
def main():
    ...
    chain_parser.add_argument(
        "--export-graph",
        action="store_true",
        help="Export context graph after execution",
    )

    # Execute module command
    module_parser = subparsers.add_parser(
        "execute-module", help="Execute a single prompt module"
    )
    ...
    diff_parser = subparsers.add_parser(
        "analyze-diff", help="Analyze a diff between two files"
    )
    ...
    diff_parser.add_argument(
        "--marker", required=True, help="Coherence marker to verify"
    )
    diff_parser.add_argument("--iteration", help="Iteration identifier")

    # Visualize context command
    viz_parser = subparsers.add_parser(
        "visualize-context", help="Visualize context graph"
    )
    viz_parser.add_argument(
        "--runtime-context", help="Path to runtime context JSON file"
    )
    viz_parser.add_argument(
        "--format",
        default="html",
    ...
    monitor_parser = subparsers.add_parser(
        "monitor-performance", help="Monitor performance metrics"
    )
    monitor_parser.add_argument(
        "--update", action="store_true", help="Update performance metrics"
    )
    monitor_parser.add_argument(
        "--iteration",
    ...
    monitor_parser.add_argument(
        "--check-compliance", action="store_true", help="Check budget compliance"
    )
    monitor_parser.add_argument(
        "--generate-dashboard",
        action="store_true",
