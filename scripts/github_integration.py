...
def create_pull_request(title, body, base_branch="main", head_branch=None):
    ...
    try:
        ...
        if not head_branch:
            head_branch = subprocess.run(['git', 'rev-parse', '--abbrev-ref', 'HEAD'], 
        ...
        try:
            ...
            subprocess.run(['gh', 'pr', 'create', 
                          ...
                          '--body', body, 
                          '--base', base_branch, 
                          '--head', head_branch], check=True)
            
            print(f"✅ Pull request created successfully.")
            return True
        except subprocess.CalledProcessError:
            print("GitHub CLI not installed. Please create PR manually or install GitHub CLI.")
...
def generate_diff_report(base_branch="main", head_branch=None):
    ...
    try:
        ...
        os.makedirs('audits/diffs', exist_ok=True)
        
        # Get changes
        diff_output = subprocess.run(['git', 'diff', '--name-status', f'{base_branch}..{head_branch}'], 
                                    check=True, capture_output=True, text=True).stdout
        ...
        # Get commit messages
        commit_output = subprocess.run(['git', 'log', '--format=%s', f'{base_branch}..{head_branch}'], 
                                      check=True, capture_output=True, text=True).stdout
        ...
        report_file = f"audits/diffs/diff_{head_branch}_{timestamp}.md"
