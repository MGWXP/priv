...
def load_registry():
    ...
    registry_path = os.path.join(os.getcwd(), 'prompt-registry.yaml')
...
def find_all_modules():
    ...
    prompt_library = os.path.join(os.getcwd(), 'prompt-library')
    if os.path.exists(prompt_library):
        module_files.extend(glob.glob(os.path.join(prompt_library, '*.md')))
    
    # Check modules directory
    modules_dir = os.path.join(os.getcwd(), 'modules')
    if os.path.exists(modules_dir):
        module_files.extend(glob.glob(os.path.join(modules_dir, '*.md')))
...
def main():
    ...
    for file_path in module_files:
        ...
        required_fields = ['name', 'version', 'marker', 'description', 'inputs', 'outputs', 'last_updated', 'status']
        ...
        if frontmatter.get('version') != registry_module.get('version'):
            ...
            print(f"  File: {frontmatter.get('version')}, Registry: {registry_module.get('version')}")
            all_valid = False
        
        # Check marker
        if frontmatter.get('marker') != registry_module.get('marker'):
            print(f"❌ ERROR: Marker mismatch for {module_name}")
            print(f"  File: {frontmatter.get('marker')}, Registry: {registry_module.get('marker')}")
            all_valid = False
        ...
        # Check outputs
        if set(frontmatter.get('outputs', [])) != set(registry_module.get('outputs', [])):
    ...
    for module_name in registry_modules:
        if module_name not in found_modules:
            print(f"❌ ERROR: Module {module_name} in registry has no corresponding file")
            all_valid = False
