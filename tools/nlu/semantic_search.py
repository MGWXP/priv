...
class SemanticSearch:
    ...
    def search(
        self, query: str, num_results: int = 5, filter_layer: Optional[str] = None
    ) -> List[Dict]:
        """
        ...
        for doc_id, embedding in self.embeddings.items():
            ...
            if (
                filter_layer
                and self.doc_registry.get(doc_id, {}).get("taxonomy_layer")
                != filter_layer
            ):
                continue
        ...
        for doc_id, score in sorted_docs[:num_results]:
            ...
            result = {
                ...
                "score": float(
                    score
                ),  # Convert from numpy to float for JSON serialization
    ...
    def _extract_snippet(self, doc_metadata: Dict, query: str) -> str:
        ...
        if "docstring" in doc_metadata:
            return (
                doc_metadata["docstring"][:150] + "..."
                if len(doc_metadata["docstring"]) > 150
                else doc_metadata["docstring"]
            )

        if (
            "front_matter" in doc_metadata
            and "description" in doc_metadata["front_matter"]
        ):
    ...
    def search_interactive(self) -> None:
        ...
        while True:
            ...
            layer_filter = input(
                "Filter by layer (config, prompts, code, docs, audit) [optional]: "
            ).strip()
            if not layer_filter:
                layer_filter = None
            ...
            try:
                num_input = input(
                    f"Number of results [default: {num_results}]: "
                ).strip()
                if num_input:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Search repository documentation semantically"
    )
    parser.add_argument(
        "--registry",
    ...
    parser.add_argument(
        "--query", help="Search query (if not provided, runs in interactive mode)"
    )
    parser.add_argument(
        "--num-results", type=int, default=5, help="Number of results to return"
    )
    parser.add_argument("--layer", help="Filter results by taxonomy layer")
    parser.add_argument(
