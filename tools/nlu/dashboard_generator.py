...
class DashboardGenerator:
    ...
    def _calculate_metrics(self) -> Dict:
        ...
        complete_prompt_modules = sum(1 for doc_id in self.doc_registry 
                                     if doc_id.startswith(("prompt-library/", "modules/")) and
                                     self.doc_registry[doc_id].get("front_matter"))
        
        # Count tested files
        tested_src_files = sum(1 for doc_id in self.doc_registry 
                               if doc_id.startswith("src/") and 
                               any(rel_type == "tested_by" and len(rels) > 0
                                   for rel_type, rels in self.relationship_map.get(doc_id, {}).items()))
        
        # Count orphaned documents
        orphaned_count = sum(1 for doc_id, relationships in self.relationship_map.items()
                           if all(len(rels) == 0 for rel_type, rels in relationships.items()))
        
        # Calculate percentages
        code_coverage = (documented_src_files / total_src_files * 100) if total_src_files > 0 else 0
        prompt_completeness = (complete_prompt_modules / total_prompt_modules * 100) if total_prompt_modules > 0 else 0
        test_coverage = (tested_src_files / total_src_files * 100) if total_src_files > 0 else 0
        
        # Calculate overall score (weighted average)
        weights = {"code_coverage": 0.4, "prompt_completeness": 0.3, "test_coverage": 0.2, "orphaned": 0.1}
        orphaned_score = 100 if orphaned_count < 5 else max(0, 100 - (orphaned_count - 5) * 10)
    ...
    def _extract_critical_issues(self) -> List[Dict]:
        ...
        try:
            ...
            core_issues = []
            core_section_match = re.search(r'## Core Document Issues(.*?)(?=##|\Z)', report_content, re.DOTALL)
            
            if core_section_match:
                core_section = core_section_match.group(1)
                issue_blocks = re.finditer(r'### (.*?)\n.*?- \*\*Path\*\*: `(.*?)`.*?- \*\*Issues\*\*:(.*?)(?=###|\Z)', 
            ...
            prompt_issues = []
            prompt_section_match = re.search(r'## Prompt Module Issues(.*?)(?=##|\Z)', report_content, re.DOTALL)
            ...
            if prompt_section_match:
                ...
                for match in issue_blocks:
                    ...
                    issues = re.findall(r'- (.*?)$', issues_text, re.MULTILINE)
                    for issue in issues:
                        if "required" in issue.lower() and "front matter" in issue.lower():
    ...
    def _generate_recommendations(self) -> List[Dict]:
        ...
        if self.metrics["code_coverage"] < 80:
            recommendations.append({
                "priority": 2 if len(recommendations) == 0 else len(recommendations) + 1,
    ...
    def generate_dashboard(self) -> str:
        ...
        for condition_match in re.finditer(r'<% if ([^%]+) %>(.*?)(?:<% else %>(.*?))?<% endif %>', dashboard, re.DOTALL):
        ...
        # Replace loops
        for loop_match in re.finditer(r'<% for (\w+) in ([^%]+) %>(.*?)<% endfor %>', dashboard, re.DOTALL):
            ...
            for item in collection:
                ...
                for var_match in re.finditer(r'<%= ' + var_name + r'\.([^%]+) %>', item_content):
...
def main():
    ...
    parser.add_argument("--validation-report", default="docs/synthesis/reports/validation_report.md", 
                        help="Path to validation report")
    parser.add_argument("--template", default="docs/nlu/dashboard_template.md", 
                        help="Path to dashboard template")
    parser.add_argument("--output", default="docs/synthesis/reports/documentation_dashboard.md", 
