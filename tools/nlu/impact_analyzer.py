...
class ImpactAnalyzer:
                        ...
                        if target in self.doc_registry:
                            if rel_type in [
                                ...
                                G.add_edge(
                                    target,
                                    doc_id,
                                    relationship=rel_type.replace("_by", ""),
                                )
    ...
    def analyze_impact(self, changed_docs: List[str], max_depth: int = 2) -> Dict:
        ...
        impact = {
            ...
            "directly_affected": [],
            "indirectly_affected": [],
            "summary": {"total_affected": 0, "by_layer": {}, "by_relationship": {}},
        }
        ...
        for doc_id in direct_affected:
            ...
            for changed_doc in doc_ids:
                for rel_type, targets in self.relationship_map.get(
                    changed_doc, {}
                ).items():
                        ...
                        relationships.append(
                            {
                                "from": changed_doc,
                                "type": rel_type,
                                "from_title": self.doc_registry.get(
                                    changed_doc, {}
                                ).get("title", changed_doc),
                            }
                        )
        ...
        if max_depth > 1:
            ...
            for depth in range(2, max_depth + 1):
                ...
                for doc_id in current_level:
                    for successor in self.graph.successors(doc_id):
                        if (
                            successor not in direct_affected
                            and successor not in doc_ids
                        ):
        ...
        total_affected = len(direct_affected) + len(
            impact.get("indirectly_affected", [])
        )
        impact["summary"]["total_affected"] = total_affected

        # Count by layer
        layers = {}
        for doc_id in direct_affected.union(
            set([d["id"] for d in impact.get("indirectly_affected", [])])
        ):
    ...
    def generate_report(self, impact: Dict) -> str:
        ...
        lines.extend(["", "### Impact by Relationship Type", ""])

        # Impact by relationship type
        for rel_type, count in impact["summary"]["by_relationship"].items():
            lines.append(
                f"- **{rel_type.replace('_', ' ').title()}**: {count} relationships"
            )
        ...
        lines.extend(["", "## Directly Affected Documents", ""])
        ...
        for doc in impact["directly_affected"]:
            ...
            for rel in doc.get("relationships", []):
                lines.append(
                    f"  - {rel['from_title']} (`{rel['from']}`) via *{rel['type'].replace('_', ' ')}*"
                )

            lines.append("")

        if impact["indirectly_affected"]:
            lines.extend(["## Indirectly Affected Documents", ""])
        ...
        if len(impact["directly_affected"]) > 10:
            lines.append(
                "- **High Impact**: These changes affect many documents. Consider breaking the changes into smaller, more focused updates."
            )

        if any(doc["layer"] == "config" for doc in impact["directly_affected"]):
            lines.append(
                "- **Config Layer Impact**: Changes affect configuration documents. Ensure all dependent systems are updated accordingly."
            )

        if any(doc["layer"] == "prompts" for doc in impact["directly_affected"]):
            lines.append(
                "- **Prompt Module Impact**: Changes affect prompt modules. Verify that AI agents using these prompts will behave as expected."
            )

        if any(doc["layer"] == "code" for doc in impact["directly_affected"]):
            lines.append(
                "- **Code Impact**: Changes affect code files. Run tests to ensure no regressions are introduced."
            )

        if len(impact["indirectly_affected"]) > len(impact["directly_affected"]):
            lines.append(
                "- **Cascade Effect**: These changes have a significant cascade effect. Review indirectly affected documents carefully."
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Analyze the impact of changes to documentation"
    )
    parser.add_argument(
        "--registry",
    ...
    parser.add_argument(
        "--documents", nargs="+", required=True, help="List of documents to analyze"
    )
    parser.add_argument(
        "--max-depth", type=int, default=2, help="Maximum depth for impact analysis"
    )
    parser.add_argument("--output", help="Output path for impact report")
    parser.add_argument(
