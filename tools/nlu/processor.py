...
class DocumentProcessor:
    ...
    def __init__(
        self,
        base_path: str,
        config_path: str = "docs/nlu/schemas/processor_config.yaml",
    ):
        """
        ...
        if not config_file.exists():
            ...
            return {
                ...
                "taxonomy_layers": [
                    {
                        "name": "config",
                        "paths": ["README.md", "AGENTS.md", "execution-budget.yaml"],
                    },
    ...
    def _extract_markdown_metadata(self, content: str, metadata: Dict) -> None:
        ...
        if headers and headers[0]["level"] == 1:
            ...
            metadata["title"] = (
                Path(metadata["file_path"]).stem.replace("_", " ").title()
            )
    ...
    def _normalize_reference(self, ref: str, source_doc_id: str) -> str:
        ...
        if (
            ...
            and not ref.endswith((".md", ".py", ".js", ".ts", ".yaml", ".yml"))
        ):
        ...
        for doc_id in self.doc_registry:
            if "tests/" in doc_id or "test_" in doc_id:
                ...
                if implementation_path:
                    ...
                    self.relationship_map[implementation_path]["tested_by"].append(
                        doc_id
                    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Process documentation and extract metadata"
    )
    parser.add_argument("--base-path", default=".", help="Base path of the repository")
    parser.add_argument(
        "--config",
        default="docs/nlu/schemas/processor_config.yaml",
        help="Path to configuration file",
    )
    parser.add_argument(
        "--output-dir", default="docs/nlu", help="Directory to save the output files"
    )
    parser.add_argument(
        "--mode",
        choices=["full", "relationships"],
        default="full",
