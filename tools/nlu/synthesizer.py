...
class DocumentSynthesizer:
    ...
    def generate_system_overview(self) -> str:
        ...
        for doc_id, relationships in self.relationship_map.items():
            for rel_type, rel_targets in relationships.items():
                if isinstance(rel_targets, list):
                    relationship_counts[rel_type] = relationship_counts.get(
                        rel_type, 0
                    ) + len(rel_targets)
        ...
        lines.extend(["", "### Relationship Statistics", ""])

        for rel_type, count in sorted(relationship_counts.items()):
            lines.append(
                f"- **{rel_type.replace('_', ' ').title()}**: {count} connections"
            )
        ...
        for doc_id, ref_count in top_docs:
            if ref_count > 0:
                ...
                lines.append(
                    f"- **{doc_title}** (`{doc_id}`): Referenced {ref_count} times"
                )
        ...
        for doc_id, relationships in self.relationship_map.items():
            if (
                len(relationships.get("references", [])) == 0
                and len(relationships.get("referenced_by", [])) == 0
            ):
    ...
    def generate_knowledge_graph(self) -> Dict:
        ...
        for doc_id, relationships in self.relationship_map.items():
            for rel_type, targets in relationships.items():
                if isinstance(targets, list) and rel_type in [
                    "references",
                    "implements",
                    "tests",
                    "documents",
    ...
    def generate_documentation_gaps(self) -> str:
        ...
        for doc_id, metadata in self.doc_registry.items():
            if metadata.get(
                "taxonomy_layer"
            ) == "code" and not self.relationship_map.get(doc_id, {}).get(
                "documented_by"
            ):
        ...
        for doc_id, metadata in self.doc_registry.items():
            if (
                metadata.get("taxonomy_layer") == "code"
                and doc_id.startswith("src/")
                and not self.relationship_map.get(doc_id, {}).get("tested_by")
            ):
        ...
        for doc_id, metadata in self.doc_registry.items():
            if doc_id.startswith(
                ("prompt-library/", "modules/")
            ) and not self.relationship_map.get(doc_id, {}).get("implemented_by"):
        ...
        if unimplemented_prompts:
            ...
            for prompt_file in sorted(unimplemented_prompts):
                doc_title = self.doc_registry.get(prompt_file, {}).get(
                    "title", prompt_file
                )
    ...
    def generate_all_reports(self, output_dir: str) -> None:
        ...
        with open(
            output_path.parent / "visualizations" / "knowledge_graph.json", "w"
        ) as f:
...
def main():
    ...
    parser.add_argument(
        "--output-dir",
        default="docs/synthesis/reports",
        help="Directory to save the reports",
    )
    parser.add_argument(
        "--report",
        choices=["all", "overview", "gaps", "xref"],
        default="all",
